...
while true; do
  ...
  if (( NOW >= DEADLINE )); then
    ...
    echo "  Check status:  xcrun notarytool info $SUBMISSION_ID --apple-id \$NOTARIZE_APPLE_ID --team-id \$DEVELOPMENT_TEAM"
    echo "  View history:  xcrun notarytool history --apple-id \$NOTARIZE_APPLE_ID --team-id \$DEVELOPMENT_TEAM"
    echo ""
